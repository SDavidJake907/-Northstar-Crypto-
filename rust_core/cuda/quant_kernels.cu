/**
 * quant_kernels.cu — GPU-accelerated quant math for Code Zero trading engine.
 *
 * Kernels:
 *   compute_returns       — simple returns from prices
 *   compute_log_returns   — log returns for Hurst exponent
 *   hurst_rs_blocks       — R/S statistic per block size per coin
 *   entropy_minmax        — find min/max returns per coin
 *   entropy_histogram     — bin returns into 10 buckets per coin
 *   autocorr_stats        — mean, variance, lag-1 covariance per coin
 *   center_rows           — subtract row-mean (for correlation/covariance)
 *   corr_normalize        — cov[i][j] / (std[i]*std[j])
 *   fingerprint_latest    — 8 market-wide metrics from returns
 *
...
/**
 * Simple returns: ret[i][j] = (price[i][j+1] - price[i][j]) / max(price[i][j], 1e-9)
 * Grid: (n_coins), Block: (min(n_returns, 256))
 */
...
/**
 * Log returns: log_ret[i][j] = log(max(price[i][j+1],1e-15)) - log(max(price[i][j],1e-15))
 * Grid: (n_coins), Block: (min(n_returns, 256))
 */
...
/**
 * Compute R/S statistic for ONE (coin, block_size) combination.
 * Each thread-block handles one (coin, block_index) pair.
 * Grid: (n_coins * n_blocks), Block: (block_size) — max 128
 *
 * rs_out[coin * MAX_SIZES + size_idx] += rs / n_blocks  (atomicAdd)
 *
 * Must zero rs_out before launch.
 */
__global__ void hurst_rs_blocks(
    ...
    // Each block handles one (coin, block_index)
    ...
    // sdata layout: [0..block_size] = block data, [block_size..2*block_size] = cum_dev
    ...
    // Compute mean (single thread for small blocks — block_size <= 128)
    ...
    // Thread 0 contributes the R/S for this block
    if (tid == 0 && s_std > 1e-15) {
        double rs = s_range / s_std;
        // Average across blocks via atomicAdd
...
/**
 * Find min and max of returns per coin.
 * Grid: (n_coins), Block: (256)
 * minmax_out[coin*2] = min, minmax_out[coin*2+1] = max
 */
...
/**
 * Bin returns into N_BINS=10 buckets per coin.
 * Grid: (n_coins), Block: (256)
 * bins_out[coin * N_BINS + bin] = count
 * Must zero bins_out before launch.
 */
#define N_BINS 10
...
__global__ void entropy_histogram(
    ...
    if (rng < 1e-15) {
        // All same value — put everything in bin 0
...
/**
 * Compute mean, variance, and lag-1 covariance of returns per coin.
 * Grid: (n_coins), Block: (256)
 * stats_out[coin*3+0] = mean, [coin*3+1] = variance, [coin*3+2] = lag1_cov
 */
__global__ void autocorr_stats(
    ...
    if (tid == 0) {
        ...
        stats_out[coin * 3 + 1] = s_var[0] / n_returns;        // variance (ddof=0)
        stats_out[coin * 3 + 2] = s_cov[0] / (n_returns - 1);  // lag-1 cov
    }
}
...
/**
 * Subtract row-mean from each element (for correlation/covariance via GEMM).
 * Grid: (n_coins), Block: (256)
 * means_out[coin] = mean of row
 */
...
/**
 * Normalize covariance matrix to correlation: corr[i][j] = cov[i][j] / (std[i]*std[j])
 * Grid: (n*n / 256 + 1), Block: (256)
 */
__global__ void corr_normalize(
    ...
    double si = stds[i];
    double sj = stds[j];
    double denom = si * sj * (double)n; // cov from GEMM is sum, not mean
...
/**
 * Compute 8 fingerprint metrics from the latest column of returns.
 * Grid: (1), Block: (256)
 * metrics_out: [r_mkt, r_btc, r_eth, breadth, median, iqr, rv_mkt, corr_avg]
 *
 * btc_returns / eth_returns: pointers to the full return series for BTC/ETH rows.
 * all_returns: full (n_coins, n_returns) matrix for correlation computation.
 */
__global__ void fingerprint_latest(
    ...
    // Single block kernel — supports up to 128 coins
    __shared__ double latest[128];   // latest return per coin
    __shared__ double sorted[128];   // for median/IQR
    __shared__ double s_rv[256];     // partial sums for rv_mkt (all threads participate)
    __shared__ double s_scalar[6];   // r_mkt, r_btc, r_eth, breadth, median, iqr
    __shared__ int    s_rv_n;        // rv window size

    int tid = threadIdx.x;

    // Load latest returns (all threads help)
    if (tid < n_coins) {
    ...
    // Thread 0: scalar stats that require serial work (sort, sums over n_coins)
    if (tid == 0) {
        double sum = 0.0;
        int pos_count = 0;
        ...
        s_scalar[0] = sum / n_coins;                                         // r_mkt
        s_scalar[1] = (btc_idx < n_coins) ? latest[btc_idx] : 0.0;          // r_btc
        s_scalar[2] = (eth_idx < n_coins) ? latest[eth_idx] : 0.0;          // r_eth
        s_scalar[3] = (double)pos_count / n_coins;                           // breadth

        // Insertion sort (n_coins is tiny)
    ...
    // ALL threads: parallel rv_mkt reduction (RMS of cross-coin avg return per timestep)
    int rv_n = s_rv_n;
    double local_sum = 0.0;
    ...
    // Thread 0: write scalar results + corr_avg (serial over n_coins)
    if (tid == 0) {
        metrics_out[0] = s_scalar[0];                                        // r_mkt
        metrics_out[1] = s_scalar[1];                                        // r_btc
        metrics_out[2] = s_scalar[2];                                        // r_eth
        metrics_out[3] = s_scalar[3];                                        // breadth
        metrics_out[4] = s_scalar[4];                                        // median
        metrics_out[5] = s_scalar[5];                                        // iqr
        metrics_out[6] = (rv_n > 0) ? sqrt(s_rv[0] / rv_n) : 0.0;          // rv_mkt

        // corr_avg: average correlation of alts to BTC (exclude BTC, ETH)
        double corr_sum = 0.0;
        int corr_count = 0;
